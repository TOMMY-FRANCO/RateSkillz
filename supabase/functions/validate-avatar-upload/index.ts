...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (file.size > MAX_FILE_SIZE) {
      return new Response(JSON.stringify({ valid: false, error: "File size must be less than 500KB. Please compress your image and try again." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
 
    // ✅ GIFs and other formats blocked at declared MIME type level
    const declaredType = file.type.toLowerCase();
    if (!ALLOWED_MIME_TYPES.includes(declaredType)) {
      return new Response(
        JSON.stringify({ valid: false, error: "Only JPEG and PNG images are allowed. GIF, SVG, and other formats are not supported." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
 
    // ✅ Magic bytes check — prevents renamed GIFs/files from slipping through
