/*
  # Fix Reward Functions to Use Correct Coin Pool Columns

  Updates all reward claiming functions to use the correct coin_pool column names:
  - Use `remaining_coins` instead of `amount`
  - Update `distributed_coins` when distributing rewards
...
BEGIN
  ...
  SELECT remaining_coins INTO v_pool_balance FROM coin_pool WHERE id = 1 FOR UPDATE;
